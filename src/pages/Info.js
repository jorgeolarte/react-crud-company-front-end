    ...
    <div className='container flex flex-col gap-5'>
      ...
      <div className='grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3'>
        {companies &&
          companies.map((item) => (
            <div
              className='grid border-2 border-white p-3 gap-3'
              key={item._id}
            >
              <div>
                <h3 className='text-lg font-semibold'>{item.name}</h3>
                <p className='text-sm font-light'>
                  <span className='font-medium'>Dirección:</span> {item.address}
                </p>
                <p className='text-sm font-light'>
              ...
              <button
                className='bg-white text-[#720cd4] hover:bg-gradient-to-tr hover:to-[#c621e5] hover:from-[#720cd4] hover:text-white p-2'
